	...
	public draw(g: CanvasRenderingContext2D): void {
		...
		CommandCenter.getInstance()
			...
			.forEach((mov) => {
				...
				g.beginPath();
				g.arc(translatedPoint.getX(), translatedPoint.getY(), 1, 0, 2 * Math.PI);
				g.fill();
			});
		...
		CommandCenter.getInstance()
			...
			.forEach((mov) => {
				...
				switch (asteroid.getSize()) {
					case 0:
						g.beginPath();
						g.arc(translatedPoint.getX(), translatedPoint.getY(), 3, 0, 2 * Math.PI);
						g.fill();
						break;
					case 1:
						g.beginPath();
					...
					default:
						g.beginPath();
						g.arc(translatedPoint.getX(), translatedPoint.getY(), 2, 0, 2 * Math.PI);
						g.stroke();
				}
			});
