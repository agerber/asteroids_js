...
import { Falcon, TurnState } from "../model/Falcon";
...
export class Game {
	// Getting the window dimensions for starting the game in fullscreen
	public static readonly DIM: Dimension = new Dimension(window.innerWidth, window.innerHeight);
	...
	keyPressed(event: KeyboardEvent): void {
		...
		switch (event.keyCode) {
			...
			case Game.KEYS.NUKE:
				...
				break;
			case Game.KEYS.UP:
				if (falcon.isThrusting()) return;
				SoundLoader.playSound("whitenoise_loop.wav");
				falcon.setThrusting(true);
				break;
			case Game.KEYS.LEFT:
				falcon.setTurnState(TurnState.LEFT);
				break;
			case Game.KEYS.RIGHT:
				falcon.setTurnState(TurnState.RIGHT);
				break;
			default:
				break;
		}
	...
	keyReleased(event: KeyboardEvent): void {
		...
		switch (event.keyCode) {
			case Game.KEYS.LEFT:
			case Game.KEYS.RIGHT:
				falcon.setTurnState(TurnState.IDLE);
				break;
			case Game.KEYS.UP:
